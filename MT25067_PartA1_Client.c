/*
 * MT25067
 * Part A1: Two-Copy TCP Client (Baseline)
 * Uses recv() socket primitive
 * Receives data from server continuously
 */
...
int main(int argc, char *argv[]) {
    ...
    printf("=== Part A1: Two-Copy Client ===\n");
    ...
    while (messages_received < num_messages) {
        ...
        while (total_recv < bytes_to_recv) {
            // TWO-COPY: recv() copies from kernel socket buffer to user buffer
